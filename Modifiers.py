...
class Modifier:
    ...
    def datetime_string_to_datetime(self, datetime_string):
        ...
        self.logger.debug("Modified " + datetime_string + " to " + str(return_datetime))
    ...
    def height_string_to_int(self, height_string):
        ...
        self.logger.debug("Modified " + height_string + " to " + str(height_int))
        return height_int
    ...
    def toi_to_sec(self, toi_string):
        ...
        self.logger.debug("Modified " + toi_string + " to " + str(seconds))
        return seconds

    def immediate(self, val):
        if val == "True":
            self.logger.debug("Parsed " + val + " as bool(True)")
            return True
        elif val == "False":
            self.logger.debug("Parsed " + val + " as bool(False)")
            return False
        try:
            return_val = int(val)
            self.logger.debug("Parsed " + val + " as int(" + str(return_val) + ")")
            return return_val
        except ValueError:
            self.logger.warning("Could not parse " + val + " as bool or int")
            return val
